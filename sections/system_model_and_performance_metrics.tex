...
%of
wavelength, azimuthal angle and elevation angle, respectively. The detail of the 2D MUSIC transform can be found in \cite{Chen2018pilot}\footnote{We modify the steering vector}.%The covariance matrix $R$ can be from measured channel or the local scattering spatial correlation model\cite{bjornson2017massive}. Moreover, $\mathbf{h}_{W}\sim \mathcal{CN}\left(\mathbf{0},\mathbf{I}_M\right)$ stands for zero-mean complex Gaussian distribution with covariance $\mathbf{I}_M$. 

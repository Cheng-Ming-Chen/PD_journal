Full frequency reuse causes significant inter-cell interference, especially for cell-edge users. Massive MIMO, under the favorable propagation condition \cite{ngo2014aspects}, can even mitigate the inter-cell interference. %harvested while coherently combines the data signal with the channel state information (CSI)
...
The two main impacts caused by pilot contamination are: first, the channel estimation degrades due to the pilot interference; second, the contaminated channel combines coherently both desired and interfered signals during the data transmission as shown in Fig.\ref{fig:PC_of_OVERLAP}. 
...
\begin{figure*} [htp]
  ...
  \subfloat[Interference during uplink pilot or data transmissions for correlated users. Not that pilot contamination is only there when the pilots are not orthogonal. ]{%
    ...
    \hfill
  \subfloat[Pilot or data interference is avoided when non-correlated users are scheduled into the network.]{%
...
We therefore propose %to not predict pilot contamination, but rather 
...
With the detection, we can reschedule the pilot sequences in an earlier phase precisely to improve channel estimation. %Second, a key point here is that if the UEs have similar channel support, by rescheduling their pilots, one can improve the channel estimation, but cannot avoid the high channel correlation in the data transmission as illustrated in Fig. \ref{fig:PC_of_DATA}.
...
$vec\{.\}$, $\otimes$ and $\arg\{.\}$ represent vectorized operation, Kronecker product and argument of a function, respectively. Finally,  $\mathbf{A} = Diag\{\mathbf{A}^{1,1},...,\mathbf{A}^{K,K}\}$ with $\mathbf{A}^{k,k}$ denoting the $k_{th}$  square matrix in the main diagonal of the block diagonal matrix $\mathbf{A}$.
